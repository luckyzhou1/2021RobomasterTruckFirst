...
{
    __HAL_TIM_CLEAR_FLAG(&TIMX,TIM_FLAG_UPDATE);//清除中断标志位 
    __HAL_TIM_SET_AUTORELOAD(&TIMX,0XFFFF);     //将重装载值设置到最大
...
{
	if(__HAL_TIM_GET_FLAG(&TIMX,TIM_FLAG_UPDATE)==SET)//在运行期间,产生了定时器溢出
	{
		usmart_dev.runtime+=0XFFFF;
	}
	usmart_dev.runtime+=__HAL_TIM_GET_COUNTER(&TIMX);
